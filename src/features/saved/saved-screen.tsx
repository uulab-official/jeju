  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <ScrollView ref={scrollRef} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {places.length ? <View style={styles.section}><Text selectable style={[styles.sectionTitle, { color: colors.text }]}>가보고 싶은 곳</Text>{places.map((place) => <PlaceCard key={place.id} place={place} />)}</View> : null}
        {words.length ? <View style={styles.section}><Text selectable style={[styles.sectionTitle, { color: colors.text }]}>기억하고 싶은 제주어</Text>{words.map((item) => <ItemRow key={`${item.kind}-${item.id}`} item={item} showKind />)}</View> : null}
      </ScrollView>
    </View>
  );
}

const styles = StyleSheet.create({ screen: { flex: 1 }, content: { paddingHorizontal: 18, paddingBottom: 34, flexGrow: 1 }, empty: { flex: 1, minHeight: 440, alignItems: 'center', justifyContent: 'center', gap: 9 }, emptyIcon: { width: 66, height: 66, borderRadius: 22, alignItems: 'center', justifyContent: 'center', marginBottom: 5 }, emptyTitle: { fontSize: 17, fontWeight: '900' }, emptyText: { maxWidth: 270, textAlign: 'center', fontSize: 13, lineHeight: 20 }, section: { gap: 12, paddingBottom: 24 }, sectionTitle: { fontSize: 19, fontWeight: '900', paddingTop: 5 } });
