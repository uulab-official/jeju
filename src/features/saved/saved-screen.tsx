...
import { ListSkeleton } from '@/src/components/ContentState';
...
export function SavedScreen() {
  ...
  const { savedIds, ready: savedReady } = useSavedPlaces();
  const { favoriteKeys, ready: favoritesReady } = useFavorites();
  ...
  const restoring = !savedReady
    || !favoritesReady
    || (favoriteKeys.size > 0 && !words.length && Object.values(resources).some((state) => state.loading));
